...
export default function StepProvincialAdminCurrentAppointment({ formData, setFormData, onValid }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [resPeo, resPmoe] = await Promise.all([
          api.get(`/register/appointment-form-data?service=${SLEAS_SERVICE_ID}&office_level=OLID003`),
          api.get(`/register/appointment-form-data?service=${SLEAS_SERVICE_ID}&office_level=OLID002`)
        ]);

        const peoData = resPeo.data;
        const pmoeData = resPmoe.data;

        const combinedOffices = [
          ...(peoData.workplacesByLevel ?? []),
          ...(pmoeData.workplacesByLevel ?? [])
        ];

        setData({
          ranks: peoData.serviceRanks ?? [],
          positions: peoData.provincialPositions ?? [],
    ...
    <div className="space-y-6 px-4 py-2">
      ...
      <div className="bg-blue-50 border border-blue-100 rounded-2xl p-4 flex gap-3">
        ...
        <p className="text-blue-800 text-sm font-medium leading-relaxed">
          Current appointment is recorded under the Sri Lanka Educational Administrative Service (SLEAS) at the Provincial Ministry or Provincial Education Office level.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <FormGroup label="Provincial Office / Ministry" required error={errors.currentAppointmentZone} className="lg:col-span-2">
          <Select
