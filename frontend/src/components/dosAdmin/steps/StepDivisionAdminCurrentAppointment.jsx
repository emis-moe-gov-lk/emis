...
// SLEAS service ID and Divisional office level
const SLEAS_SERVICE_ID = "SER005";
const DIVISIONAL_OFFICE_LEVEL = "OLID005";
...
export default function StepDivisionAdminCurrentAppointment({ formData, setFormData, onValid }) {
    ...
    const fetchData = async () => {
      try {
        const res = await api.get(
          `/register/appointment-form-data?service=${SLEAS_SERVICE_ID}&office_level=${DIVISIONAL_OFFICE_LEVEL}`,
        );
        const d = res.data;
        setData({
    ...
    <div className="space-y-6 px-4 py-2">
      ...
      <div className="bg-blue-50 border border-blue-100 rounded-2xl p-4 flex gap-3">
        ...
        <p className="text-blue-800 text-sm font-medium leading-relaxed">
          Current appointment is recorded under the Sri Lanka Educational Administrative Service (SLEAS) at the Divisional Education Office level.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <FormGroup label="Divisional Education Office" required error={errors.currentAppointmentZone} className="lg:col-span-2">
          <Select
