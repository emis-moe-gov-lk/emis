...
import { getDivisionDeoFormData } from "@/api/divisionDeoService";
...
export default function StepDivisionCurrentAppointment({ formData, setFormData, onValid }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const data = await getDivisionDeoFormData({ service: formData.currentAppointmentService || "" });

        setServices(data.services ?? []);
        setRanks(data.serviceRanks ?? []);
        setPositions(data.positions ?? []);
        setDivisionalOffices(data.divisionalOffices ?? []);
      } catch (error) {
  ...
  return (
    <div className="space-y-2 animate-in fade-in slide-in-from-bottom-4 duration-500 px-6 py-0 [&_input]:bg-white dark:[&_input]:bg-gray-800 [&_input]:bg-white dark:[&_input]:bg-gray-800 [&_select]:bg-white dark:[&_select]:bg-gray-800 [&_textarea]:bg-white dark:[&_textarea]:bg-gray-800 [&_label]:text-xs [&_label]:font-bold [&_label]:text-gray-700 dark:[&_label]:text-gray-300">
