...
const PendingConfirmationList = () => {
  ...
  useEffect(() => {
    setLoading(true);
    api.get(`/alerts/pending-confirmation?per_page=20&page=${currentPage}`)
      .then((res) => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Pending Confirmation</h1>
          <p className="text-gray-500 dark:text-gray-400 text-sm mt-1">Teacher profiles awaiting confirmation</p>
        </div>
        ...
        <div className="space-y-4">
          {filteredTeachers.length > 0 ? (
            <>
              {filteredTeachers.map((teacher) => (
                <div
                  ...
                  className="group flex flex-col md:flex-row md:items-center gap-4 p-5 bg-white dark:bg-gray-800 rounded-2xl shadow-sm hover:shadow-md border border-gray-100 dark:border-gray-700 transition-all duration-200 hover:border-green-100 dark:hover:border-green-900/30 cursor-pointer"
                >
                  <div className="flex items-center gap-4 min-w-[60px]">
                    <div className="p-2.5 bg-green-50 dark:bg-green-900/20 rounded-xl text-green-600 dark:text-green-400 group-hover:scale-110 transition-transform">
                  ...
                  <div className="flex-1 min-w-0 grid grid-cols-1 md:grid-cols-12 gap-4 items-center">
                    <div className="md:col-span-4">
                      <h3 className="text-lg font-bold text-gray-900 dark:text-white truncate group-hover:text-green-600 transition-colors">
                        {teacher.full_name}
                      </h3>
                    ...
                    <div className="md:col-span-2 flex items-center justify-end gap-2">
                      <Badge color="info" className="px-3 py-1 whitespace-nowrap">Verified</Badge>
                      <Button
                        size="xs"
                        color="dark"
                        onClick={(e) => {
            ...
            <div className="text-center py-20 bg-white dark:bg-gray-800 rounded-3xl border border-gray-100 dark:border-gray-700">
              ...
              <p className="text-gray-500 max-w-sm mx-auto mt-2">No teachers pending confirmation.</p>
            </div>
          )}
        </div>
      )}
