 ...
 * - Pre-loads existing MOE Administrator data if found
 ...
 * - Returns: Existing MOE Administrator data or 404 if not found
 * 
 * @component
 * @param {Object} props
...
export default function StepNICVerification({
   ...
   * MOE Administrators should be within 100 years of current date
   * 
   * @param {string} value - NIC number to validate
  ...
  const verifyNIC = useCallback(async () => {
    ...
    try {
      ...
      if (nicAvailable) {
        ...
        // Warn if user already has active appointment
