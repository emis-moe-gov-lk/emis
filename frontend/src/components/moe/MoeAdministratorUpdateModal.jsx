...
export default function MoeAdministratorUpdateModal({
	isOpen,
	section,
	adminId,
	onClose,
	...
	useEffect(() => {
		...
		console.log(`[MoeAdministratorUpdateModal] Loading data for section: ${section}, adminId: ${adminId}`);

		api
			.get(`/moe-administrators/${adminId}`)
			.then(async (res) => {
				if (ignore || res.data?.status !== "success") return;
				const d = res.data.data;
				console.log("[MoeAdministratorUpdateModal] Data loaded:", d);
			...
			.catch((err) => {
				console.error("[MoeAdministratorUpdateModal] Load error:", err);
	...
	const handleSave = async () => {
		...
		try {
			...
			console.log(`[MoeAdministratorUpdateModal] Saving section: ${section}, payload:`, payload);

			const res = await api.patch(`/moe-administrators/${adminId}`, payload);
			console.log("[MoeAdministratorUpdateModal] Save response:", res.data);

			toast.success("Updated successfully");
			onClose();
			onSaved?.(res.data);
		} catch (error) {
			console.error("[MoeAdministratorUpdateModal] Save error:", error);
