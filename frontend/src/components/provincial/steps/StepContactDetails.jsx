...
export default function StepContactDetails({ formData, setFormData, onValid, apiErrors = {} }) {
  ...
  const errors = useMemo(() => {
    ...
    if (!formData.contact) {
      ...
      newErrors.contact = "Invalid contact number (must be 10 digits starting with 0)";
    }
    ...
    if (formData.latitude?.trim()) {
      ...
      if (!VALIDATION_PATTERNS.COORDINATE.test(lat)) {
        ...
        newErrors.latitude = "Latitude must be within Sri Lanka range (5.9 to 9.9)";
      }
    }
    ...
    if (formData.longitude?.trim()) {
      ...
      if (!VALIDATION_PATTERNS.COORDINATE.test(lng)) {
        newErrors.longitude = "Longitude must be decimal format (e.g., 79.8612)";
      } else if (
        Number.isNaN(lngNum) ||
        lngNum < SRI_LANKA_BOUNDS.LNG_MIN ||
        lngNum > SRI_LANKA_BOUNDS.LNG_MAX
      ) {
        newErrors.longitude = "Longitude must be within Sri Lanka range (79.5 to 81.9)";
      }
    }
    ...
    <div className="flex justify-center px-4 py-2">
      <div className="w-full max-w-4xl space-y-6">
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <FormGroup label="Address Line 1" required error={getFieldError("addressLine1")}>
              <TextInput
            ...
            <FormGroup label="Address Line 2" required error={getFieldError("addressLine2")}>
              <TextInput
