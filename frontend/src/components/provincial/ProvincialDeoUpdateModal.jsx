...
export default function ProvincialDeoUpdateModal({
	isOpen,
	section,
	deoId,
	onClose,
	...
	}, [isOpen, deoId, onClose]);

	useEffect(() => {
		if (!isOpen || !deoId || !section) return;
		...
		api
			...
			.catch((err) => {
				console.error("[ProvincialDeoUpdateModal] Load error:", err);
				toast.error("Failed to load DEO data");
			})
			.finally(() => {
	...
	const handleSave = async () => {
		...
		try {
			...
			const res = await api.put(`/provincial-deos/${deoId}`, payload);

			toast.success("Updated successfully");
			onClose();
			onSaved?.(res.data);
		} catch (error) {
			console.error("[ProvincialDeoUpdateModal] Save error:", error);
