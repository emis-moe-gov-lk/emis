...
export const ProvincialDeoFormContext = createContext();

const STORAGE_KEY = "provincial_deo_form_draft";
...
export function ProvincialDeoFormProvider({ children }) {
  ...
  useEffect(() => {
    ...
    if (savedState) {
      try {
        ...
        console.error("Failed to restore Provincial DEO form state:", error);
  ...
  return (
    <ProvincialDeoFormContext.Provider value={{ state, dispatch }}>
      {children}
    </ProvincialDeoFormContext.Provider>
  );
}
