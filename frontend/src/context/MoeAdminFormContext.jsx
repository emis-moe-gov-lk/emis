...
export const MoeAdminFormContext = createContext();

const STORAGE_KEY = "moe_admin_form_draft";
...
export function MoeAdminFormProvider({ children }) {
  ...
  useEffect(() => {
    ...
    if (savedState) {
      try {
        ...
        console.error("Failed to restore MOE Admin form state:", error);
  ...
  return (
    <MoeAdminFormContext.Provider value={{ state, dispatch }}>
      {children}
    </MoeAdminFormContext.Provider>
  );
}
