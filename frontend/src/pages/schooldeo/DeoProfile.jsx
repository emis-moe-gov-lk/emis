    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <section className="lg:col-span-9 space-y-5">
          ...
          {activeTab === "edit" && <EditRequestTab editRequests={editRequests} />}
        </section>
      </div>
...
function EditRequestTab({ editRequests }) {
  return (
    <div className="space-y-5">
      ...
      <div className="rounded-2xl border surface p-6 text-sm">{editRequests?.length ? (<ul className="space-y-3">{editRequests.map((r) => (<li key={r.id} className="rounded-2xl border p-4"><strong>{r.title}</strong><p>{r.note}</p></li>))}</ul>) : <div className="text-gray-500">No requests found.</div>}</div>
    </div>
  );
}
