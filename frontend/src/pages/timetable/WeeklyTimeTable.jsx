...
function WeeklyTimetable() {
  ...
  const moveInterval = async (interval, direction) => {
    ...
    const ivIndex = items.findIndex(
      (it) => it.type === "interval" && it.startTime === interval.startTime,
    );
    if (ivIndex === -1) return;
    ...
    const periodBeforeIdx = periodItems.findIndex(
      (p) => p.endTime === interval.startTime,
    );
  ...
  const changeIntervalDuration = async (interval, delta) => {
    const currentDuration = minutesBetween(
      interval.startTime,
      interval.endTime,
    );
  ...
  const IntervalRow = ({ item }) => {
    ...
    const interval = intervals.find(
      (iv) => iv.startTime === item.startTime && iv.endTime === item.endTime,
    );
    ...
    const periodBeforeIdx = periodItems.findIndex(
      (p) => p.endTime === item.startTime,
    );
    const canMoveUp = periodBeforeIdx > 0;
    const canMoveDown =
      periodBeforeIdx >= 0 && periodBeforeIdx < periodItems.length - 2;
          ...
          <div className="flex items-center gap-3">
            ...
            {!isLocked && interval ? (
              <div className="flex items-center gap-2">
                ...
                <span
                  className={`text-[11px] font-bold uppercase tracking-wider ${isSaving ? "text-amber-300" : "text-amber-500"}`}
                >
                  ...
                  return (
                      ...
                      onClick={async () => {
                        ...
                        if (!holiday && slotsOnDay.length > 0) {
                          if (
                            !confirm(
                              `There are ${slotsOnDay.length} slot(s) on ${day}. Mark as holiday anyway?`,
                            )
                          )
                            return;
                        }
                    ...
                    >
                      ...
                      >
                        ...
                        {holiday && (
                          <span className="ml-1 font-semibold">(Holiday)</span>
                        )}
                      </div>
                    </th>
                  );
                    ...
                    <tr className="border-t">
                      ...
                      {ALL_DAYS.map((day, dayIndex) => {
                        ...
                        if (isOff && !slot) {
                          return (
                            ...
                            >
                              ...
                              >
                                <span
                                  className={`font-medium ${!isLocked ? "text-orange-400" : "text-red-300"}`}
                                >
                        ...
                        if (isOff && slot) {
                          ...
                          const hasRecord =
                            recordedDates[slot.id]?.includes(dayDateISO);
                          return (
                            <td
                              key={day}
                              ...
                              >
                                ...
                                <div className="flex items-center gap-1.5 mt-1">
                                  ...
                                  <button
                                    ...
                                    title={
                                      hasRecord ? "Edit record" : "Add record"
                                    }
                                  >
                                    <svg
                                      className="w-3.5 h-3.5"
                                ...
                                {slot.purpose && (
                                  <p
                                    className="mt-1.5 text-[10px] text-orange-600/80 italic truncate"
                                    title={slot.purpose}
                                  >
                                    {slot.purpose}
                                  </p>
                        ...
                        return (
                          <td
                            key={day}
                            className="px-3 py-3 h-0 min-w-[calc(100vw-130px)] sm:min-w-0 sm:w-[160px]"
                          >
                            {slot ? (
                              (() => {
                                const color = subjectColors[slot.subject];
                              ...
                              >
                                {!isLocked && (
                                  <span className="text-violet-500">+ Add</span>
                                )}
                              </div>
                            )}
                          </td>
            ...
            <div className="px-5 py-4 border-b bg-white dark:bg-gray-900/10 flex items-center justify-between shrink-0">
              <div>
                ...
                {selectedDay === todayName && (
                  <p className="text-xs text-emerald-500 font-medium">Today</p>
                )}
              </div>
              <button
                onClick={() => setSelectedDay(null)}
