...
import { getDivisionAdminById, addDivisionAdminServiceHistoryEntry, addDivisionAdminPastService } from "@/api/divisionAdminService";
...
export default function DivisionAdminProfile() {
  const { id } = useParams();
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const res = await getDivisionAdminById(id);
  ...
  const handlePastServiceSave = async () => {
    ...
    try {
      ...
      const res = await addDivisionAdminPastService(id, { ...pastServiceForm, office_level_id: officeLevelId });
      if (res?.status === "success") {
  ...
  const handleServiceHistorySave = async () => {
    ...
    try {
      ...
      const res = await addDivisionAdminServiceHistoryEntry(id, payload);
      if (res.status === "success") {
          ...
          <div className="rounded-2xl surface overflow-hidden">
            <div className="px-4 py-3 border-b border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-900/70">
              ...
              <div className="text-xs text-gray-500 dark:text-gray-400">
                Division Administration profile
              </div>
            </div>
      ...
      <div className="bg-linear-to-r from-blue-50 to-indigo-50/30 dark:from-blue-900/10 dark:to-indigo-900/5">
        <div className="p-6 flex flex-col xl:flex-row xl:items-center gap-6">
          ...
          <div className="flex flex-col sm:flex-row xl:flex-col gap-3 min-w-[200px]">
            <Can permission={PermissionGroups.DIVISION.ADMIN_PROFILE_VIEW}>
