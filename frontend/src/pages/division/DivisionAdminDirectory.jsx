...
export default function DivisionAdminDirectory() {
  ...
  useEffect(() => {
    const fetchEmployees = async () => {
      try {
        setLoading(true);
        const response = await getAllDivisionAdmins();
        if (response.status === "success") {
          setEmployees(response.data);
        } else {
          setError(response.message || "Failed to fetch division administrators");
        }
      } catch (err) {
        setError(err.message || "An error occurred while fetching division administrators");
      } finally {
        setLoading(false);
      }
    };
