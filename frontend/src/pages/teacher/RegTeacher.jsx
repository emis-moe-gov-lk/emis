...
const REG_TEACHER_TOTAL_STEPS = 6;
...
function RegTeacherInner() {
  ...
  const clampStep = (value) => {
    ...
    return Math.min(Math.max(Math.trunc(parsed), 1), REG_TEACHER_TOTAL_STEPS);
  };

  const isRegTeacherHistoryState = (historyState) =>
    Boolean(
      historyState &&
      historyState.__regTeacherOwner === REG_TEACHER_HISTORY_OWNER &&
      Number.isFinite(Number(historyState[REG_TEACHER_HISTORY_STEP_KEY])),
    );

  const buildRegTeacherHistoryState = (step) => {
    const baseState = window.history.state || {};
    return {
      ...baseState,
      __regTeacherOwner: REG_TEACHER_HISTORY_OWNER,
      [REG_TEACHER_HISTORY_STEP_KEY]: clampStep(step),
    };
  };
  ...
  useEffect(() => {
    ...
    if (!isRegTeacherHistoryState(baseState)) {
      window.history.replaceState(buildRegTeacherHistoryState(initialStep), "", currentUrl);
    } else if (clampStep(baseState[REG_TEACHER_HISTORY_STEP_KEY]) !== initialStep) {
      window.history.replaceState(
        buildRegTeacherHistoryState(initialStep),
        "",
        currentUrl,
      );
    }

    // Add a guard entry to prevent browser Back from leaving this route unexpectedly.
    window.history.pushState(buildRegTeacherHistoryState(initialStep), "", currentUrl);
    ...
    const handlePopState = async (event) => {
      ...
      if (!isRegTeacherHistoryState(event.state)) {
        window.history.pushState(
          buildRegTeacherHistoryState(lastHistoryStepRef.current || currentUiStep || 1),
          "",
          currentUrl,
        );
        return;
      ...
      const stepFromHistory = clampStep(event.state[REG_TEACHER_HISTORY_STEP_KEY]);
      ...
      if (stepFromHistory === 1 && currentUiStep > 1) {
        if (hasDraftData) {
          ...
          if (!result.isConfirmed) {
            window.history.pushState(
              buildRegTeacherHistoryState(currentUiStep),
              "",
              currentUrl,
            );
  ...
  useEffect(() => {
    ...
    window.history.pushState(
      buildRegTeacherHistoryState(normalizedStep),
      "",
  ...
  const confirmDiscardAndRun = async (
    ...
    const result = await Swal.fire({
      title: "Are you sure?",
      text: "Saved teacher registration draft will be lost. Do you want to continue?",
      icon: "info",
      showCancelButton: true,
      confirmButtonColor: "#3085d6",
      cancelButtonColor: "#d33",
