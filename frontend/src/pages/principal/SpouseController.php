...
class SpouseController extends Controller
{
    public function store(Request $request, string $people_id)
    {
        ...
        if (\App\Models\Spouse::where('people_id', $people_id)->where('status', 'Active')->exists()) {
    ...
    public function update(Request $request, string $people_id, $spouse_id)
    {
        ...
        if ($validated['status'] === 'Active') {
            $activeExists = \App\Models\Spouse::where('people_id', $people_id)
                ->where('status', 'Active')
                ->where('id', '!=', $spouse_id)
                ->exists();
            ...
            if ($activeExists) {
                return response()->json([
                    'status' => 'error',
                    'message' => 'An active spouse record already exists for this person. Please update or change the current active spouse first.',
                ], 422);
            }
        }
