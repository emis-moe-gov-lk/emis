...
import MoeAdminUpdateModal from "@/components/moe/MoeAdminUpdateModal";
...
export default function MoeAdminProfile() {
  const { id } = useParams();
  ...
  const loadProfile = async () => {
    try {
      setLoading(true);
      const res = await getMoeAdminById(id);
  ...
  const handlePastServiceSave = async () => {
    ...
    try {
      ...
      const officeLevelId = isSchoolBasedService(serviceName) ? "OLID006" : "OLID001";
      const res = await addMoeAdminPastService(id, { ...pastServiceForm, office_level_id: officeLevelId });
      if (res?.status === "success") {
  ...
  const handleServiceHistorySave = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        office_level_id: isOfficeBased ? "OLID001" : "OLID006",
      };

      const res = await addMoeAdminServiceHistoryEntry(id, payload);
      if (res.status === "success") {
              ...
              <button 
                onClick={async () => {
                  try {
                    ...
                    link.href = url;
                    link.download = `moe-admin-profile-${profile.nic || profile.employeeId}.pdf`;
  ...
  return (
    <div className="rounded-3xl overflow-hidden border border-gray-100 dark:border-gray-700 bg-white dark:bg-gray-800 shadow-sm">
      ...
      >
        ...
        {right && <div className="text-white [&_button]:border-white/20 [&_button]:bg-white/10 [&_button]:text-white hover:[&_button]:bg-white/20 hover:[&_button]:border-white/35 [&_button]:shadow-none [&_button_svg]:text-white">{right}</div>}
      </div>
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between gap-3">
        ...
        <Can permission={PermissionGroups.MOE.ADMIN_WOP}>
          <RoundedActionButton onClick={() => onEdit("wop")}>Edit</RoundedActionButton>
        </Can>
      </div>
