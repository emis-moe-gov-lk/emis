...
import { HiArrowLeft, HiEye, HiEyeOff, HiUserAdd } from "react-icons/hi";
import { useNavigate } from "react-router-dom";
import { toast } from "react-hot-toast";
import { getRoles } from "@/api/roleService";
import { createUser, parseUserApiError } from "@/api/userService";
...
const UserCreate = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const { message, fieldErrors } = parseUserApiError(error, "Unable to create user.");
    ...
    <div className="p-4 sm:p-6 lg:p-10 max-w-4xl mx-auto">
      <div className="rounded-3xl overflow-hidden border border-gray-200 dark:border-gray-700 shadow-sm bg-white dark:bg-gray-800">
        <div className="bg-gradient-to-r from-cyan-600 via-sky-600 to-blue-700 px-6 sm:px-8 py-8 text-white">
          ...
          <h1 className="mt-2 text-2xl sm:text-3xl font-bold">User Create</h1>
          <p className="text-cyan-100 mt-2">Create user profile and account</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="px-6 sm:px-8 py-7 space-y-7">
          ...
          <div className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-slate-50/70 dark:bg-slate-800/50 p-4 sm:p-5">
            ...
            {errors.roles && (
              <p className="text-sm text-red-600 mt-2">{errors.roles}</p>
            )}
          </div>
