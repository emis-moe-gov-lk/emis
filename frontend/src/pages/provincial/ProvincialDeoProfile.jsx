...
export default function ProvincialDeoProfile() {
  const { id } = useParams();
  ...
  const handlePastServiceSave = async () => {
    ...
    try {
      ...
      const res = await addProvincialDeoPastService(id, { ...pastServiceForm, office_level_id: officeLevelId });
      if (res?.status === "success") {
  ...
  const handleServiceHistorySave = async () => {
    ...
    try {
      ...
      const res = await addProvincialDeoServiceHistoryEntry(id, payload);
      if (res.status === "success") {
  ...
  const handleDownloadDocument = async () => {
    const peopleId = deo?.people_id ?? id;
    if (!peopleId) return;

    setIsDownloadingDocument(true);
    try {
      const response = await downloadProvincialDeoProfileDocument(peopleId);
      ...
      link.href = url;
      link.download = `provincial-deo-profile-${deo?.nic || peopleId}.pdf`;
      document.body.appendChild(link);
      link.click();
      link.remove();
      window.setTimeout(() => window.URL.revokeObjectURL(url), 1000);
    } catch (error) {
      console.error("Failed to download DEO document:", error);
      toast.error("Unable to download the DEO document.");
    } finally {
          ...
          <div className="rounded-2xl surface overflow-hidden">
            <div className="px-4 py-3 border-b border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-900/70">
              ...
              <div className="text-xs text-gray-500 dark:text-gray-400">
                Provincial Development Officer profile
              </div>
            </div>
      ...
      <div className="bg-linear-to-r from-blue-50 to-indigo-50/30 dark:from-blue-900/10 dark:to-indigo-900/5">
        <div className="p-6 flex flex-col xl:flex-row xl:items-center gap-6">
          ...
          <div className="flex flex-col sm:flex-row xl:flex-col gap-3 min-w-[200px]">
            <Can permission={PermissionGroups.ZONAL.DEO_PROFILE_VIEW}>
              <button
