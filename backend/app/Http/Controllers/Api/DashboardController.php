...
class DashboardController extends Controller
{
    public function __invoke(Request $request, string $people_id)
    {
        ...
        \Log::debug($people_id);
        \Log::debug(auth()->user()->people_id);

        if (auth()->user()->people_id !== $people_id) {
            return response()->json(['status' => 'error', 'message' => 'Forbidden2'], 403);
        }
        ...
        $jwtRoles = $request->attributes->get('jwt_roles', []);
        $jwtRole  = is_array($jwtRoles) ? ($jwtRoles[0] ?? null) : $jwtRoles;

        $roles = $user ? $user->roles->pluck('name')->values()->all() : [];
        $dbRole   = $roles[0] ?? null;

        // Converting to lowercase
        $jwtRole = $jwtRole ? strtolower($jwtRole) : null;
        $dbRole  = $dbRole ? strtolower($dbRole) : null;
