    ...
    public function update(Request $request, $people_id)
    {
        try {
            ...
            try {
                ...
                Log::error('Provincial Admin Update WSO2 IS sync error', ['message' => $e->getMessage()]);
            }

            return response()->json([
                'status'  => 'success',
                'message' => 'Provincial admin updated successfully',
                'data'    => $people->fresh(),
            ], 200);
            ...
            DB::rollBack();
            Log::error('Provincial Admin Update Error', ['error' => $e->getMessage()]);

            return response()->json([
                'status'  => 'error',
                'message' => 'Failed to update Provincial admin',
            ], 500);
        }
    }
