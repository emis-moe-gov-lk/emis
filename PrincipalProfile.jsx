...
const PrincipalProfile = () => {
  ...
  if (!principal) {
    return (
      <div className="flex flex-col items-center justify-center py-20 bg-white dark:bg-gray-800 rounded-3xl shadow-sm border border-gray-100 dark:border-gray-700 m-6">
        <HiExclamation className="h-12 w-12 text-rose-500 mb-4" />
        <p className="text-gray-500 dark:text-gray-400">
          Failed to load principal profile data.
        </p>
        <Button onClick={loadPrincipalProfile} className="mt-4" color="light">
          Retry
        </Button>
      </div>
    );
...
function FamilyTab({ family, onAddSpouse, onEditSpouse, onAddChild, onEditChild, onDeleteChild }) {
    ...
    <div className="space-y-10">
      ...
      <div className="space-y-5">
        <div className="flex items-center justify-between gap-3">
          ...
          {!spouses.some((s) => s.status === "Active") && (
